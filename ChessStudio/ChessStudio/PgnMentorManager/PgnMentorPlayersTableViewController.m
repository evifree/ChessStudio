//
//  PgnMentorPlayersTableViewController.m
//  ChessStudio
//
...
@interface PgnMentorPlayersTableViewController () {
...
@implementation PgnMentorPlayersTableViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = NSLocalizedString(@"PGN_MENTOR_PLAYERS", nil);
    
    self.navigationController.navigationBar.barTintColor = UIColorFromRGB(0XFAFAD2);
...
{
    static NSString *CellIdentifier = @"Cell Pgn Players";
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    fileToDownload = [listPlayersPgn objectAtIndex:indexPath.row];
    linkFile = [listLinkPlayers objectAtIndex:indexPath.row];
    
    //NSLog(@"%@    %@    %@", dato, dato2, linkFile);
...
- (void) connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    [saveDatabase setFileToSave:[listPlayersPgn objectAtIndex:[self.tableView indexPathForSelectedRow].row]];
