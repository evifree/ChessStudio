...
extern void engineSendMove(const std::string &best, const std::string &ponder);
extern std::string engineSendAnalysis(const Position& pos, std::vector<Search::RootMove> &roots, std::size_t size, long depth, Value value, long nps);
...
void MainThread::search() {
  ...
  if (bestThread != this)
    ...
    //const std::string r = UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());
    //pv_to_ui2(r);
...
string UCI::pv(const Position& pos, Depth depth, Value alpha, Value beta) {
  ...
  for (size_t i = 0; i < multiPV; ++i)
  {
      ...
      if (i == 0)
      {
          std::vector<Search::RootMove> temp = rootMoves;
          std::string s = engineSendAnalysis(pos, temp, Options["MultiPV"], d, v, pos.nodes_searched() * 1000 / elapsed);
          pv_to_ui2(s);
      }
