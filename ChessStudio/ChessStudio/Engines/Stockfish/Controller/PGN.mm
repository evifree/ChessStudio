...
-(id)initWithFilename:(NSString *)aFilename {
   ...
   file = fopen([[PGN_DIRECTORY stringByAppendingPathComponent: aFilename]
                  UTF8String], "r");
   if (file) {
      filename = [[PGN_DIRECTORY stringByAppendingPathComponent: aFilename] retain];
   } else {
...
-(void)initializeGameIndices {
   ...
   [self filterByPlayerName: searchString];
    ...
    //NSLog(@"PGN:Trovate %d partite", numberOfGames);
}
...
-(NSString *)eco {
    return [NSString stringWithUTF8String: eco];
}
