...
moves_loop: // When in check and at SpNode search starts from here
    ...
    while ((move = mp.next_move<SpNode>()) != MOVE_NONE)
    {
      ...
      if (RootNode)
      {
          ...
          if (thisThread == Threads.main() && Time::now() - SearchTime > 3000)
              ...
              // PUSH iPHONE
              //currmove_to_ui(move_to_uci(move, false), (int)moveCount + (int)PVIdx, (int)RootMoves.size(), int(depth / ONE_PLY));
  ...
  // POP iPHONE
    ...
    // PUSH iPHONE
    string ios_pv(const Position& pos, int depth, int value, int alpha, int beta, Move pv[]) {
  ...
  string uci_pv(const Position& pos, int depth, Value alpha, Value beta) {
    ...
    for (size_t i = 0; i < uciPVSize; ++i)
    {
        ...
        // PUSH iPHONE
        //pv_to_ui(pv_to_ios(&RootMoves[i].pv[0]), d, score_to_ios(v), scoretype_to_ios(v, alpha, beta), abs(v) >= VALUE_MATE_IN_MAX_PLY);
        
        pv_to_ui2(ios_pv(pos, int(d), int(v), int(alpha), int(beta), &RootMoves[i].pv[0]));
        // POP iPHONE        
    }
