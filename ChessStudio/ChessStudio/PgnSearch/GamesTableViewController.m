...
- (void) viewWillAppear:(BOOL)animated {
    ...
    NSMutableString *titolo = [[NSMutableString alloc] initWithString:NSLocalizedString(@"GAMES OF", nil)];
    [titolo appendString:_playerName];
    if (IS_PHONE) {
        ...
        self.navigationItem.title = [NSString stringWithFormat:@"%@ in %@", titolo, _pgnFileDoc.pgnFileInfo.fileName];
    }
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    //NSLog(@"Numero partite = %lu", (unsigned long)_games.count);
    return _games.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *CellIdentifier = @"Cell GamesTable";
    ...
    //NSLog(@"Numero matches %d", matches.count);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //gameSel = [gameSel stringByReplacingOccurrencesOfString:@"][" withString:replaceSeparator];
    
    //NSLog(@"GAME SEL  %@", gameSel);
...
- (BOOL) checkTheGame:(NSIndexPath *) indexPath {
    ...
    if (indexPath) {
        ...
        indicePartita = [[_pgnFileDoc.pgnFileInfo getAllGamesAndTags] indexOfObject:gameSel];
    }
...
- (void) aggiorna:(PGNGame *)pgnGame {
    ...
    [_games replaceObjectAtIndex:selectedIndexPath.row withObject:updatedGame];
    [self.tableView reloadData];
    if (_delegate) {
        [_delegate aggiorna];
    }
        ...
        if ([title isEqualToString:NSLocalizedString(@"MENU_COPY_GAMES", nil)]) {
            if (partiteSelezionateDaCopiareEliminare.count > 0) {
                ...
                for (NSIndexPath *indexPath in partiteSelezionateDaCopiareEliminare) {
                    NSString *gameAndTags = [_games objectAtIndex:indexPath.row];
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView.tag == 100) {
        ...
        if ([title isEqualToString:@"OK"]) {
            ...
            for (NSIndexPath *indexPath in partiteSelezionateDaCopiareEliminare) {
                NSString *gameToDelete = [_games objectAtIndex:indexPath.row];
                [gamesToDelete addObject:gameToDelete];
            }
            [_games removeObjectsInArray:gamesToDelete];
