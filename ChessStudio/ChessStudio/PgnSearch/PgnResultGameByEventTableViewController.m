//
//  PgnResultGameByEventTableViewController.m
//  ChessStudio
//
...
@interface PgnResultGameByEventTableViewController () {
    
    NSString *pattern;
    NSRegularExpression *regex;
    
    
    NSMutableDictionary *gamesByEvent;
...
@implementation PgnResultGameByEventTableViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"Games by events";
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    if (IS_PHONE) {
        ...
        self.navigationItem.title = NSLocalizedString(@"GAMES_BY_EVENTS_INFO", nil);
        return;
        ...
        if (IS_ITALIANO) {
            ...
            label1 = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 160, 28)];
        ...
        label1.text = [NSString stringWithFormat:NSLocalizedString(@"GAMES_BY_EVENT_TABLE_VIEW_CONTROLLER_TITLE", nil), @""];
        ...
        NSString *titolo = [NSString stringWithFormat:NSLocalizedString(@"GAMES_BY_EVENT_TABLE_VIEW_CONTROLLER_TITLE", nil), _pgnFileDoc.pgnFileInfo.fileName];
...
- (void) initData {
    ...
    //NSLog(@"EventCountedSet = %d", eventsCountedSet.count);
    ...
    gamesByEvent = [[NSMutableDictionary alloc] init];
    NSUInteger maxEventiConsentiti = _pgnFileDoc.pgnFileInfo.numberOfGames.integerValue / 3;
    //NSLog(@"MaxEventiConsentiti = %d", maxEventiConsentiti);
    if (eventArray.count > maxEventiConsentiti) {
        //UIAlertView *maxNumEventiAlertView = [[UIAlertView alloc] initWithTitle:nil message:@"Numero di Eventi troppo elevato rispetto al numero di partite!" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        //[maxNumEventiAlertView show];
        //return;
        _maxEventi = YES;
    }
    ...
    for (NSString *event in eventArray) {
        ...
        //NSString *selectedEvent = [event stringByReplacingOccurrencesOfString:@"][EventDate" withString:@"]|[EventDate"];
        
        
        
        NSString *selectedEvent = [event stringByReplacingOccurrencesOfString:@"][" withString:replaceSeparator];
        ...
        k++;
        //NSLog(@"%d       Devo trovare partite con parametro =    %@", k, parametro);
        
        NSArray *gamesFound = [_pgnFileDoc.pgnFileInfo findGamesByTagValues:parametro];
        
        //NSLog(@"%d       Per evento %@ trovate %d partite", k, event, gamesFound.count);
        
        [gamesByEvent setObject:gamesFound forKey:event];
    }
    
    //NSLog(@"Terminato INIT DATA");
}
...
{
    //NSString *event = [[_gamesByEvent allKeys] objectAtIndex:section];
    //NSString *event = [_events objectAtIndex:section];
    //NSArray *games = [_gamesByEvent objectForKey:event];
    
    NSString *event = [eventArray objectAtIndex:section];
    
    return [eventsCountedSet countForObject:event];
}

/*
- (NSString *) tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    //return [[_gamesByEvent allKeys] objectAtIndex:section];
    
    NSString *event = [eventArray objectAtIndex:section];
    NSMutableString *header = [[NSMutableString alloc] init];
    [header appendString:event];
    [header appendString:@" "];
    [header appendFormat:@"(%d games)", [eventsCountedSet countForObject:event]];
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    //label.backgroundColor = UIColorFromRGB(0x1f2229);
    ...
    //label.text = [@"  " stringByAppendingString:[[_gamesByEvent allKeys] objectAtIndex:section]];
    //label.text = [@"  " stringByAppendingString:[_events objectAtIndex:section]];
    
    NSString *event = [[eventArray objectAtIndex:section] stringByReplacingOccurrencesOfString:@"][" withString:replaceSeparator];
    NSArray *event1Array = [event componentsSeparatedByString:separator];
    NSMutableString *header = [[NSMutableString alloc] init];
    for (int i=0; i<event1Array.count; i++) {
        NSString *ev = [event1Array objectAtIndex:i];
        NSArray *evArray = [ev componentsSeparatedByString:@"\""];
        NSString *eventSenzaApici = [evArray objectAtIndex:1];
        if (header.length == 0) {
            [header appendString:@"  "];
        }
        else {
        ...
        [header appendString:eventSenzaApici];
    }
    
    NSUInteger numGamesByEvent = [eventsCountedSet countForObject:[eventArray objectAtIndex:section]];
    
    if (numGamesByEvent == 1) {
        [header appendFormat:NSLocalizedString(@"NUM_GAMES_SINGOLAR", @"1 partita"), numGamesByEvent];
    }
    else {
        [header appendFormat:NSLocalizedString(@"NUM_GAMES_PLURAL", @"n partite"), numGamesByEvent];
    }
...
{
    static NSString *CellIdentifier = @"Cell Games";
    ...
    if (gamesByEvent.count > 0) {
        NSString *event = [eventArray objectAtIndex:indexPath.section];
        
        NSArray *games = [gamesByEvent objectForKey:event];
        game = [games objectAtIndex:indexPath.row];
        ...
        if ([eventData rangeOfString:@"|"].length == 0) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (gamesByEvent.count > 0) {
        _games = [gamesByEvent objectForKey:selectedEvent];
        gameSel = [_games objectAtIndex:indexPath.row];
...
- (BOOL) checkTheGame:(NSIndexPath *) indexPath {
    ...
    @try {
        if ([PGNGame gameIsPositionWithRegularFen:gameSel]) {
            ...
            [_games replaceObjectAtIndex:indexPath.row withObject:gameSel];
            if ([PGNGame gameIsPositionWithRegularNumbering:gameSel]) {
                pgnGame = [[PGNGame alloc] initWithPgn:gameSel];
...
- (PGNGame *) getNextGame {
    if (lastSelectedGame == (_games.count - 1)) {
        return nil;
    }
    lastSelectedGame++;
    gameSel = [_games objectAtIndex:lastSelectedGame];
    pgnGame = [[PGNGame alloc] initWithPgn:gameSel];
    return pgnGame;
}
        ...
        if ([title isEqualToString:NSLocalizedString(@"MENU_COPY_GAMES", nil)]) {
            if (partiteSelezionateDaCopiareEliminare.count > 0) {
                ...
                for (NSIndexPath *indexPath in partiteSelezionateDaCopiareEliminare) {
                    ...
                    NSString *_gameSel = [games objectAtIndex:indexPath.row];
        ...
        if ([title isEqualToString:NSLocalizedString(@"MENU_DELETE_GAMES", nil)]) {
            if (partiteSelezionateDaCopiareEliminare.count>0) {
                ...
                for (NSIndexPath *indexPath in partiteSelezionateDaCopiareEliminare) {
                    ...
                    NSArray *games = [gamesByEvent objectForKey:selectedEvent];
                    NSString *_gameSel = [games objectAtIndex:indexPath.row];
                    [gamesToDelete addObject:_gameSel];
                }
                NSString *msg;
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag == 200 || alertView.tag == 300) {
        ...
        if ([scelta isEqualToString:NSLocalizedString(@"MENU_CANCEL", nil)]) {
            ...
            NSInteger *indexGame = [_games indexOfObject:gameSel];
            [_games replaceObjectAtIndex:indexGame withObject:newGameSel];
            gameSel = newGameSel;
