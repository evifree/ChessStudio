//
//  SquaresStyleTableViewController.m
//  ChessStudio
//
...
@interface SquaresStyleTableViewController () {
...
@implementation SquaresStyleTableViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = NSLocalizedString(@"BOARD", nil);
    
    contents = [UtilToView getTipoSquares];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [settingManager squaresAsString:[contents objectAtIndex:indexPath.row]];
    cell.imageView.image = [UIImage imageNamed:[contents objectAtIndex:indexPath.row]];
    cell.imageView.transform = CGAffineTransformMakeScale(0.8, 0.8);
    if ([[contents objectAtIndex:indexPath.row] isEqualToString:[settingManager squares]]) {
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row != checkedRow) {
        ...
        NSString *ts = [contents objectAtIndex:indexPath.row];
        [settingManager setSquares:ts];
        if (_delegate) {
            [_delegate updateFromSquaresStyle];
        }
    }
